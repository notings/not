...
function Carouselimg(opt) {
    ...
    //找节点
    this.box = document.getElementsByClassName(this.dafaultData.ele);
    this.list = document.getElementsByClassName('imglist')[0];
    this.light = document.getElementsByClassName('light')[0];
    // this.prevBtn =document.getElementsByClassName('prev')[0];
    // this.nextBtn = document.getElementsByClassName('next')[0];
    // this.posibtn = document.getElementsByClassName('posibtn')[0];
    this.timer = null;
    this.num = 0; //可视区图片的下标
...
Carouselimg.prototype.init = function () {
    ...
    // html=`<span>国庆：致敬经典丛书专场</span>
    // <span>展商：2019潘家园首届古旧书博览会</span>
    // <span>拍卖：陶雅世家</span>
    // <span>书单：武术</span>
    // <span>公告：杂书馆收购图书文献</span>
    // <span>书店：一苇书房</span>
    // <span>书单：魏晋书风</span>
    // <span>书店：一是斋</span>`
...
Carouselimg.prototype.movelight = function (ev) {
    if (ev.target.tagName.toLowerCase() == 'span') {
        ...
        if (index > this.num) {
            //新图从右侧进入
            //旧图挪走
            startMove(this.imglist[this.num], {
                'left': -this.iw
            });
            this.imglist[index].style.left = this.iw + 'px';
            startMove(this.imglist[index], {
                'left': 0
            });
        }
        if (index < this.num) {
